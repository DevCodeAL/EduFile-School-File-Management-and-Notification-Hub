...
export default function AdminNews(){
  ...
  // Fetch news Files
    ...
    // Handle Input Change
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await fetch(`http://localhost:5000/api/newsItem-files`, {
            method: "POST",
            body: data,
          });
  ...
  const filePreview = () => {
    ...
    return(
        <>
             <div className="flex flex-col min-h-screen bg-gray-100">
                 ...
                 <main className="flex-1 p-6 ml-64">
                   <h1 className="text-2xl font-bold text-gray-700 mb-4">📰  News</h1>
                   ...
                   {isOpen && (
                     <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm  z-50">
                       <div className="bg-white p-6 shadow-lg rounded-lg max-w-lg w-full relative">
                         ...
                         <h2 className="text-lg font-semibold mb-4 text-gray-700">Create News Updates</h2>
        ...
        {news.length === 0 ? (
            <p className="text-gray-500 text-center col-span-full">No news yet.</p>
        ) : (
            news.slice().reverse().map((newItem) => (
            <div
                key={newItem._id}
