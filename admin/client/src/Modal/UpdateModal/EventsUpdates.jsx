...
import { createUpdateEvents } from "../../Services/ItemServices";

export default function EventsUpdate({isOpenEvents, isCloseEvents, newItem, onUpdate}) {
...
// Modal Options
  ...
  const HandleSubmitUpdate = async (e) => {
    ...
    try {
      const response = await createUpdateEvents(newItem._id, data);
      onUpdate(response);
      isCloseEvents();
    } catch (error) {
  ...
  return (
          <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm  z-50">
            <div className="bg-white p-6 shadow-lg rounded-lg max-w-lg w-full relative">
              {/* Close Button */}
              <button
                onClick={isCloseEvents}
                className="absolute top-2 right-2 text-gray-600 hover:text-red-500 text-xl"
              >
                ✕
              </button>

              <h2 className="text-lg font-semibold mb-4 text-gray-700">Update Events</h2>
