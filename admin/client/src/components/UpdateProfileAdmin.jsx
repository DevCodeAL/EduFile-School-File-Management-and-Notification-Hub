...
import { updateProfileAPI } from "../Services/ItemServices";
const VITE_API_BASE_URL = import.meta.env.VITE_API_BASE_URL;

const UpdateProfileAdmin = ({isOpenProfile, isCancel})=> {
  const { user } = useAuth();
  const [isLoadingUpdate, setLoadingUpdate] = useState(false);
  const [isFormData, setFormData] = useState({ picture: null, preview: null, fullname: '', email: '', contact: '', school: ''});
  ...
  const HandleUpdateProfileUpdate = async(e)=>{
    ...
    data.append("school", isFormData.school);
...
<div className="fixed z-10 inset-0 flex items-center justify-center bg-black bg-opacity-50">
  <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
    ...
    <div className="space-y-3">
      ...
      <input type="text" name="school" value={isFormData.school} onChange={handleChange} placeholder="School Name" className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500"/>
    </div>
